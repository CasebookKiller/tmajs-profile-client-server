  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      const res:User = await getData(
        `http://${HOST}:${PORT}/login`, data.email, data.password
      )
      if (res) {
        console.log(res);
        ...
        >
          <div className='flex flex-column px-8 py-5 gap-4' style={{ borderRadius: '12px', backgroundImage: 'radial-gradient(circle at left top, var(--primary-400), var(--primary-700))' }}>
                  ...
                  <Controller name='email'
                    ...
                    render={({ field, fieldState }) => (
                      <InputText
                        ...
                        autoFocus
                        {...field}
                        className={classNames('bg-white-alpha-20 border-none p-3 text-primary-50',{ 'p-invalid': fieldState.invalid })}
                      />
                  )} />
            ...
            <div className='flex align-items-center gap-2'>
              <Button label='Войти' type='submit' className='p-3 w-full text-primary-50 border-1 border-white-alpha-30 hover:bg-white-alpha-10'></Button>
              <Button label='Отмена' onClick={(e) => hide(e)} text className='p-3 w-full text-primary-50 border-1 border-white-alpha-30 hover:bg-white-alpha-10'></Button>
            </div>
          </div>
        </form>
      )}
